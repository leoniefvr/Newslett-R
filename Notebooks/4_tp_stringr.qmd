...
```{webr}
#| label: substring
df %>% select(id_siae,siret_signature) %>% mutate(siren_signature=str_sub(siret_signature,1,9)) %>% head(10)
```
...
df %>% transmute(as.character(siret_actualise),comptage_zero=str_count(siret_actualise,"0")) %>% head(10)
...
df %>% transmute(as.character(siret_actualise),comptage_zero=str_count(siret_actualise,"00")) %>% head(10)
