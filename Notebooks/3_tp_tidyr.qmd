...
resources:
  - ../data/donnees_synthetiques_structure_iae.parquet
engine: knitr
---
...
df <- read_parquet("../data/donnees_synthetiques_structure_iae.parquet")
```

```{webr}
#| label: vue colonnes
df %>% select(id_siae,annee,effectif)
```
...
```{webr}
#| label: pivot wider
df_wider <- df %>% pivot_wider(id_cols=id_siae,names_from = annee ,values_from = effectif , names_prefix = "effectif_")
df_wider
```
...
df_wider %>%
  rowwise() %>%
  mutate(total_effectif = sum(c_across(starts_with("effectif_")), na.rm = TRUE))
```
...
```{webr}
#| label: vue siret
df %>% select(id_siae,siret_signature,siret_actualise)
```
